...
class GraphImpl[VD: ClassTag, ED: ClassTag] protected (
  ...
  override def aggregateMessagesWithActiveSet[A: ClassTag](
    ...
    // 对边的EdgePartion 进行mapPartition操作
    val preAgg = view.edges.partitionsRDD.mapPartitions(_.flatMap {
      case (pid, edgePartition) =>
        ...
        //到这里完成了 本地的map和aggregate 消息
    }).setName("GraphImpl.aggregateMessages - preAgg")
    //这里返回的好像是那个messagerdd的迭代器。
  ...
  override def outerJoinVertices[U: ClassTag, VD2: ClassTag]
    ...
    if (eq != null) {
      ...
      //vertex 还外来的messagerdd 进行leftjoin
...
object GraphImpl {
  ...
  def apply[VD: ClassTag, ED: ClassTag](
    ...
    /**
      * 调用fromEdgeRDD 生成GraphImpl ，调用前先使用EdgeRDD.fromEdges 生成EdgeRDD。
      */
  ...
  def fromExistingRDDs[VD: ClassTag, ED: ClassTag](
      ...
      edges: EdgeRDD[ED]): GraphImpl[VD, ED] = {
    /**
      * 实例化GraphImpl生成图，其中注意的是Graph里 包含的是vertex，和 ReplicatedVertexView 这个里面包含了EdgeRDDImpl
      *
      */
  ...
  private def fromEdgeRDD[VD: ClassTag, ED: ClassTag](
      ...
      vertexStorageLevel: StorageLevel): GraphImpl[VD, ED] = {
    /**
      * 将上面生成的EdgeRDDImPl先缓存下来 便于计算。
      */
    val edgesCached = edges.withTargetStorageLevel(edgeStorageLevel).cache()
    /**
      *   根据EdgeRDD创建VertexRDD。
      */
 val vertices =
