...
object GraphLoader extends Logging {
    ...
    : Graph[Int, Int] =
  {
    ...
    // lines 通过textfile读数据，形成linesrdd  然后 根据numedgepartition的数量 觉得是否进行分区
    val lines =
    ...
    //这里是对lines rdd 针对每个partition进行操作。 partition以迭代器的形式出现。随州返回一个迭代器，每个迭代器里有一个元组，每个元组是pid ，edgepartition
