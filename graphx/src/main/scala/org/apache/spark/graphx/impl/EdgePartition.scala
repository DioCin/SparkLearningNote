...
class EdgePartition[
   ...
   * 沿着边进行发送消息，在收到的点的部分进行聚合，对所有边进行顺序扫描。
  ...
  def aggregateMessagesEdgeScan[A: ClassTag](
    ...
    var i = 0
    //遍历 获取到src的attr 还有dst的attr。然后发送消息。
    while (i < size) {
      val localSrcId = localSrcIds(i)
      val srcId = local2global(localSrcId)
      val localDstId = localDstIds(i)
      ...
      if (edgeIsActive) {
        ...
        //调用的是AggregatingEdgeContext 里的method  最终调用的是send  实际上在进行aggregates 数组里相同点的信息的本地聚合。
...
每个点之间在发消息的时候是独立的，即：点单纯根据方向，向以相邻点的localId为下标的数组中插数据，互相独立，在并行上互不影响。 完事，返回消息RDDmessages: RDD[(VertexId, VD2)]
    ...
    //返回消息RDDmessages: RDD[(VertexId, VD2)]
