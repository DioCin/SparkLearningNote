...
object EdgeRDD {
  ...
  def fromEdges[ED: ClassTag, VD: ClassTag](edges: RDD[Edge[ED]]): EdgeRDDImpl[ED, VD] = {
    /**
      * 对RDD[Edge[ED]]进行mappartition操作，遍历其Edge对象的值，利用EdgePartitionbuilder，将值加入builder的edges向量里。调用builder.toEdgePartition 生成EdgePartition 返回对应的包含pid和分区的迭代器。
      */
